...
class AdminWithdrawRequestDataTable extends DataTable
{
    /**
     * Build the DataTable class.
     *
     * @param QueryBuilder<AdminWithdrawRequest> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                $view = '<a href="' . route('admin.withdraw-request.show', $query->id) . '" class="btn btn-purple mx-2">
                        View
                      </a>';
         ...
         ->addColumn('current_balance', function ($query) {
                ...
                $totalEarnings = Order::where('order_status', 'delivered')
                    ...
                    ->whereHas('orderProducts', function ($subQuery) use ($query) {
                        $subQuery->where('vendor_id', $query->vendor_id);
                    })->sum('sub_total');

                // Get current balance from the latest 'paid' withdrawal or use total earnings
                $currentBalance = null;
                $paidRequest = WithdrawRequest::where('vendor_id', $query->vendor_id)
                    ->where('status', 'paid')
                    ->orderBy('id', 'DESC')
                    ->first();
                ...
                return $settings->currency_icon . ($currentBalance ?? 'N/A');
            })
     ...
     *
     * @return QueryBuilder<AdminWithdrawRequest>
     */
    ...
    {
        return $this->builder()
            ->setTableId('adminwithdrawrequest-table')
    ...
    {
        return 'AdminWithdrawRequest_' . date('YmdHis');
    }
}
