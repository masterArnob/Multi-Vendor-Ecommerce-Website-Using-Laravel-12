...
class VendorProductVariantDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
                ...
                $edit = '<a href="' . route('vendor.product-variant.edit', ['product_id' => request()->product_id ,'variant_id' => $query->id]) . '" class="btn btn-warning mx-2">
                        Edit
                      </a>';
                $delete = '<a href="' . route('vendor.product-variant.destroy', $query->id) . '" class="btn btn-danger delete-item">
                        Delete
                      </a>';
