...
use App\DataTables\AdminVendorProductImageGalleryDataTable;
...
class VendorProductImageGalleryController extends Controller
{
     ...
     */
    public function index(AdminVendorProductImageGalleryDataTable $dataTable)
    {
        $product_id = request()->product_id;
        $product = Product::findOrFail($product_id);
        return $dataTable->render('admin.product.vendor-image-gallery.index', compact('product'));
    }
    ...
    if ($request->hasFile('image')) {
        foreach ($request->file('image') as $file) {
            ...
            $gallery->admin_id = 0;
            $gallery->vendor_id = $product->vendor_id;
            $gallery->save();
        }
    }
