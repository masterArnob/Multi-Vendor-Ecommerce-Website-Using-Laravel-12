                ...
                <div class="container-xl">
                    <div class="row g-2 align-items-center">
                        ...
                        <div class="col-auto ms-auto d-print-none">
                            <a href="{{ route('vendor.order.index') }}" class="btn btn-primary">Back</a>
...
$total = 0;
                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <select name="order_status" data-id="{{ $order->id }}" class="form-control order_status">
                                    ...
                                    <option value="processed_and_ready_to_ship" @selected($order->order_status === 'processed_and_ready_to_ship')>
                                        Processed and Ready to Ship</option>
                                    <option value="dropped_off" @selected($order->order_status === 'dropped_off') disabled>Dropped Off</option>
                                    <option value="shipped" @selected($order->order_status === 'shipped') disabled>Shipped</option>
                                    <option @selected($order->order_status === 'out_for_delivery') disabled>Out For Delivery
                                    </option>
                                    <option @selected($order->order_status === 'delivered') disabled>Delivered</option>
                                    <option @selected($order->order_status === 'canceled') disabled>Canceled</option>
                                </select>
                                </div>
        ...
        //dd($product->variant_total);
        $total += $product->unit_price * $product->qty + ($product->variant_total ?? 0);
                                    @endphp
                                            <tr>
    ...
    <script>
               var config = {
            routes: {
                changeOrderStatus: "{{ route('vendor.change-order-status') }}", // Changed to route() for consistency
            },
            icon: {
