@php
    $singleCatTwo = json_decode($singleCatTwo->value);

    // Determine the deepest non-null category and its source
    $categoryDisplay = $singleCatTwo[0]->category;
    $categorySource = 'category'; // Default source
    $cat = App\Models\Category::find($singleCatTwo[0]->category);
    $products = App\Models\Product::where(['category_id' => $cat->id])->get();

    if (!is_null($singleCatTwo[0]->sub_category)) {
        $categoryDisplay = $singleCatTwo[0]->sub_category;
        $categorySource = 'sub_category';
        $cat = App\Models\SubCategory::find($singleCatTwo[0]->sub_category);
        $products = App\Models\Product::where(['sub_category_id' => $cat->id])->get();
    }
    if (!is_null($singleCatTwo[0]->child_category)) {
        $categoryDisplay = $singleCatTwo[0]->child_category;
        $categorySource = 'child_category';
        $cat = App\Models\ChildCategory::find($singleCatTwo[0]->child_category);
